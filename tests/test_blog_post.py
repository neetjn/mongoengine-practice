...
class BlogPostTests(TestCase):
    ...
    def test_search_post(self):
        ...
        self.simulate_post(
            ...
            body=to_json(PostFormDtoSerializer, generate_post_form_dto()),
            headers=self.headers)
        target_post = generate_post_form_dto()
        self.simulate_post(
            PostCollectionResource.route,
            body=to_json(PostFormDtoSerializer, target_post),
            headers=self.headers)
        search_settings = PostSearchSettingsDto(
            query=target_post.title,
            options=[PostSearchOptions.TITLE])
        ...
        found_post = post_search_res.json.get('posts')[1]
        self.assertEqual(target_post.title, found_post.get('title'))
        self.assertEqual(target_post.description, found_post.get('title'))
        self.assertEqual(target_post.content, found_post.get('title'))
